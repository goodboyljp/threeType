...
(function ($) {
    ...
    $.fn.mdatetimer = function(opts){
		var defaults = {
			...
			years : [2017,2018], //年份，数组或字符串'2000~2015'
			nowbtn : true,
			onOk : null,
			onCancel : null
		};
		...
		var F = {
			...
			formatDate : function(year, month, day, hour, minute, second){
				...
				if(option.mode!=3&&option.mode!=5){
				...
				if(option.format==1){
					...
					if(option.mode!=1 && hour){
                        ...
                        if(option.mode==5){
                            result += '00';
                        }else if(option.mode!=5){
                            result += minute+'分';
                            if(option.mode!=2&&option.mode!=4){
			...
			var picker = {
				init : function(){
					...
					for(var j=0; j<12; j++){
						...
						monthStr += '<li class="'+sel+'" data-month="'+(j)+'">'+F.preZero(j+1)+'月</li>';
					}
                    ...
                    if(option.mode==5){
                        //把不该显示的删除掉
                        $(".mt_year, .mt_month, .mt_day, .mt_hour, .mt_minute, .mt_second").css("width","25%");
                        $('.mt_body', container).css('height', 200);
                        $('.mt_year, .mt_month, .mt_day, .mt_hour', container).css('height', '100%');
                        $('.mt_sepline, .mt_minute, .mt_second, .mt_indicate.cate2', container).remove();
                    }
					if(!option.nowbtn){
						$('.mt_setnow', container).remove();
						$('.mt_cancel, .mt_ok', container).css('float', 'none');
						$('.mt_cancel', container).css('border-right', '1px solid #fdb700');
					}

                    $('.sel').click(function(){
                        document.addEventListener('touchmove', function (e) { e.preventDefault(); }, false);
                    })
                    $('.mt_confirm a').click(function(){
